...
const emptyForm = () => ({ name: '', description: '', website: '', is_active: true })
...
function openEdit(vendor) {
    ...
    form.value      = { name: vendor.name, description: vendor.description || '', website: vendor.website || '', is_active: !!vendor.is_active }
    errors.value    = {}
    resetImageState()
    currentImage.value = vendor.logo || null
    showModal.value = true
}
...
async function save() {
    ...
    try {
        ...
        if (editMode.value) {
            await axios.put(`/admin/vendors/${savedId}`, form.value)
        } else {
            const { data } = await axios.post('/admin/vendors', form.value)
            savedId = data.id
        }
        ...
        if (imageFile.value) {
            ...
            await axios.post(`/admin/vendors/${savedId}/image`, fd, {
            ...
            <table v-else class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    <tr v-for="v in vendors" :key="v.id" class="hover:bg-gray-50">
                        <td class="px-6 py-3">
                            <img v-if="v.logo" :src="v.logo" class="w-8 h-8 object-contain rounded-lg border border-gray-200 bg-white p-0.5" />
                            <div v-else class="w-8 h-8 rounded-lg bg-primary-50 flex items-center justify-center text-primary text-xs font-bold">{{ v.name[0] }}</div>
                        </td>
                        <td class="px-6 py-4">
                    ...
                    <tr v-if="vendors.length === 0">
                        <td colspan="5" class="px-6 py-10 text-center text-muted">Вендоров пока нет</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
            <div class="bg-white rounded-2xl w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
                ...
                <form @submit.prevent="save" class="p-6 space-y-4">
                    ...
                    <!-- Логотип -->
                    <div>
                        <label class="block text-sm font-medium text-dark mb-2">Логотип</label>
                        <div v-if="currentImage && !imagePreview" class="mb-3 flex items-start gap-3">
                            <img :src="currentImage" class="w-20 h-20 object-contain rounded-lg border border-gray-200 bg-gray-50 p-1" />
                            <span class="text-xs text-muted mt-1">Текущий логотип</span>
                        </div>
                        <div v-if="imagePreview" class="mb-3 flex items-start gap-3">
                            <img :src="imagePreview" class="w-20 h-20 object-contain rounded-lg border border-gray-200 bg-gray-50 p-1" />
